...
func (driver *Driver) Config(config map[string]interface{}) error {
...
func (driver *Driver) CreateNetwork(nid types.UUID, config map[string]interface{}) error {
...
func (driver *Driver) CreateEndpoint(nid, eid types.UUID, config map[string]interface{}) (*sandbox.Info, error) {
...
// EndpointInfo is an implementation of driverapi.Driver.EndpointInfo
// in terms of remote driver
func (driver *Driver) EndpointInfo(nid, eid types.UUID) (map[string]interface{}, error) {
	reader, err := driver.remote.Call("GET", fmt.Sprintf("%s/%s", nid, eid), nil)
	if err != nil {
		logrus.Warningf("Driver returned err:", err)
		return nil, err
	}
	...
	return info, json.NewDecoder(reader).Decode(&info)
}

// Join is an implementation of driverapi.Driver.Join in terms of
// remote driver
func (driver *Driver) Join(nid, eid types.UUID, sboxKey string, options map[string]interface{}) (*driverapi.JoinInfo, error) {
	reader, err := driver.remote.Call("POST", fmt.Sprintf("%s/%s/join", nid, eid), options)
	if err != nil {
		logrus.Warningf("Driver returned err:", err)
		return nil, err
	}
...
// Leave is an implementation of driverapi.Driver.Leave in terms of
// remote driver
func (driver *Driver) Leave(nid, eid types.UUID, options map[string]interface{}) error {
	reader, err := driver.remote.Call("POST", fmt.Sprintf("%s/%s/leave", nid, eid), options)
	if err != nil {
		logrus.Warningf("Driver returned err:", err)
		return err
	}
